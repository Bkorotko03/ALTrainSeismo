...
// ------------------------------------ Buzzer -----------------------
  ...
  analogReadResolution(14); // 14 bit read resolution, to convert to voltage mult by 5/16383
...
// time to parse the GPS data
  if (millis() - lastLogTime >= LOG_INTERVAL_MS) { // if its been at least a log interval, do below
    ...
    if (GPS.year >= 25 && GPS.year <= 27) { // we wait until we get a gps lock to do any kind of recording or naming
      ...
      if (!fileInitialized && GPS.year >= 0 && GPS.year <= 50) { // check if file not initialized and sanity check that the GPS data makes sense
